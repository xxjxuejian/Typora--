...
`default`的效果如下，分别为`H5`和微信小程序中的显示效果
...
在H5端存在`statusBarHeight`属性，但是值为0，所以通常设置一个默认值

在微信小程序端， 状态栏高度 `statusBarHeight：44` 单位是`px`
...
在H5端根本没有这个方法，不能直接使用，要么就直接给一个固定值

在微信小程序中，可以获取到胶囊按钮盒子的 `高度height` 以及距离 页面顶部的`top值`
