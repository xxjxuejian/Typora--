...
这里为大家介绍一个非常好用的包组成可视化工具——[webpack-bundle-analyzer](https://link.juejin.cn/?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Fwebpack-bundle-analyzer)，配置方法和普通的 plugin 无异，它会以矩形树图的形式将包内各个模块的大小和依赖关系呈现出来，格局如官方所提供这张图所示：

![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b9e2b9389a6b41ca99ee0730914c4e33~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp)

在使用时，我们只需要将其以插件的形式引入：

```javascript
javascript
复制代码const BundleAnalyzerPlugin = require('webpack-bundle-analyzer').BundleAnalyzerPlugin;
...
```javascript
javascript
复制代码import { page1, page2 } from './pages'
...
```javascript
javascript
复制代码const UglifyJsPlugin = require('uglifyjs-webpack-plugin');
module.exports = {
 plugins: [
   new UglifyJsPlugin({
     // 允许并发
...
```javascript
javascript
复制代码import BugComponent from '../pages/BugComponent'
...
...
```javascript
javascript
复制代码const getComponent => (location, cb) {
...
```javascript
javascript
复制代码require.ensure(dependencies, callback, chunkName)
```
