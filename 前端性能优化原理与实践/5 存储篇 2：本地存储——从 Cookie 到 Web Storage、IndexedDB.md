...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/9/22/166002bebabcf363~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
```ini
ini
复制代码Set-Cookie: name=xiuyan; domain=xiuyan.me
```
...
```javascript
javascript
复制代码localStorage.setItem('user_name', 'xiuyan')
```
...
```javascript
javascript
复制代码localStorage.getItem('user_name')
```
...
```javascript
javascript
复制代码localStorage.removeItem('user_name')
```
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/9/22/16600d07e6abdd81~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2018/9/29/16623d7a9457d1df~tplv-t2oaga2asx-zoom-in-crop-mark:3024:0:0:0.awebp)
...
```javascript
javascript
复制代码  // 后面的回调中，我们可以通过event.target.result拿到数据库实例
  let db
  // 参数1位数据库名，参数2为版本号
...
```javascript
javascript
复制代码// onupgradeneeded事件会在初始化数据库/版本发生更新时被调用，我们在它的监听函数中创建object store
...
浏览器缓存/存储技术的出现和发展，为我们的前端应用带来了无限的转机。近年来基于缓存/存储技术的第三方库层出不绝，此外还衍生出了 [PWA](https://link.juejin.cn/?target=https%3A%2F%2Flavas.baidu.com%2Fpwa) 这样优秀的 Web 应用模型。可以说，现代前端应用，尤其是移动端应用，之所以可以发展到在体验上叫板 Native 的地步，主要就是仰仗缓存/存储立下的汗马功劳。