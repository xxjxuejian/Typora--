...
`content script` API 只允许注入 `JavaScript` 或 `CSS` 文件，这意味着当内容脚本中需要显示比较复杂的HTML布局时，需要完全用`JS`对象来编写它，这编写起来很长，很难维护，而且绝对不是设计师友好的。

##### 1. 注入 HTML 文件内容 (推荐)

这是一种常见且相对简单的方法，可以在保持 HTML、CSS 和 JavaScript 分离的同时，将复杂的 UI 注入到页面中。
